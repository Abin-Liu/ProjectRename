...
namespace MFGLib
{
	...
	public sealed class RegistryHelper : IDisposable
	{
		...
		/// <summary>
		/// Constructor with subkey path
		/// </summary>
		/// <param name="subKey">Path of the subkey, e.g. "Software\\Microsoft\\Windows"</param>
		/// <param name="writable">True if the subkey sould be opened writable, false if readonly</param>
		public RegistryHelper(string subKey, bool writable = false)
		{
			Open(subKey, writable);
		}

		/// <summary>
		/// Constructor with company and product name
		/// </summary>
		/// <param name="company">Name of thew company, e.g. "Microsoft"</param>
		/// <param name="product">Name of the product, e.g. "Windows"</param>
		/// <param name="writable">True if the subkey sould be opened writable, false if readonly</param>
		public RegistryHelper(string company, string product, bool writable = false)
		{
			Open(company, product, writable);
		}
		#endregion
		...
		public bool Open(string subKey, bool writable = false)
		{
			...
			try
			{
				...
				{
					m_regKey = Registry.CurrentUser.OpenSubKey(subKey);
				}
			}
			catch
			{
		...
		public void DeleteSubKey(string subKey)
		{
			...
			try
			{
				m_regKey.DeleteSubKeyTree(subKey, false);
			}
			catch
			{
			}
		...
		public static void AddAutoStartApp(string name, string executablePath, string parameters = null)
		{
			...
			using (RegistryHelper reg = new RegistryHelper("Software\\Microsoft\\Windows\\CurrentVersion\\Run", true))
			{
		...
		/// </summary>
		/// <param name="name">Application name</param>
		public static void RemoveAutoStartApp(string name)
		{
			using (RegistryHelper reg = new RegistryHelper("Software\\Microsoft\\Windows\\CurrentVersion\\Run", true))
			{
